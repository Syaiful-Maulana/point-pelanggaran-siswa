  ...
  <div class="modal fade" id="modal-info" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form action="kategori.store" method="POST" id="forms">
            @csrf
        <div class="modal-body">
              <div class="form-group">
                <label for="">Kategori Pelanggaran</label>
                <input type="text" class="form-control" autocomplete="off" id="kategori" name="kategori" placeholder="Kategori Pelanggaran">
...
<script>
    ...
    $(document).on('click', '.edit', function(){
        $('#forms').attr('action', "{{ route('kategori.update')}}")
        let id = $(this).attr('id')
        $.ajax({
            ...
            success: function(res){
                ...
                $('#modal-info').on("hidden.bs.modal", function(){
                    $('#forms')[0].reset()
                    $('#forms').attr('action', "{{ route('kategori.store')}}")
                    })
            },
            error: function(xhr){
                console.log(xhr)
...
<script>
    $(document).on('click', '.add', function(){
        let id = $(this).attr('id')
        $("a").attr("href", "/detail_pelanggaran/" + id)
    })
</script>
